     ...
     function downloadMp3(url, fileName){
      ...
      function getVideoDetails (item, _fun) {
        ...
        .execute(function(response) {
          ...
          if(!(hora > 0 || minuto > 20 )){
            console.log(item,response,time)
      ...
      function playSong(item){
        ...
        getUrl(item,function(url,item){
            ...
            $("#player").data("data",JSON.stringify({id:item.id.videoId}));
      ...
      function nuevaLista () {
        ...
        if(listas.where({nombreLista: nombre}).length == 0){
          listas.push(newList);
      ...
      function agregarALista (nombre,item) {
        var theList = localStorage.getItem(nombre);
        if(theList.where({"id.videoId":currentOptionPressed.id.videoId}).length == 0){
